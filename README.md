...
| [hunyuan-3d-material](skills/hunyuan-3d-material/SKILL.md) | 占位，待接入 | 后续补充纹理、UV 和材质相关需求 |
| [hunyuan-3d-animation](skills/hunyuan-3d-animation/SKILL.md) | 占位，待接入 | 后续补充绑骨蒙皮和动作相关需求 |

材质和动画技能当前没有可执行功能，不会提交生成任务。
...
`hunyuan-3d-mesh` 必须与 `hunyuan-3d-generator` 一同安装，保持以下结构；网格客户端从相邻生成技能复用鉴权、提交、轮询和下载实现。
...
当前共 113 项离线测试（生成客户端 57 项，网格客户端 56 项），模拟网络并阻止实际连接。两项占位技能仅做结构校验。测试通过不代表云端模型权限、额度或资产质量已验证。
